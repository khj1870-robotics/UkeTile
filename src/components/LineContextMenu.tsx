...
/** Popup shown when a sheet line number is held still — duplicate/delete/reorder that line. */
export function LineContextMenu({ x, y, canDelete, onDuplicate, onDelete, onReorder, onDismiss }: Props) {
    ...
    <>
      ...
      <View style={[styles.menu, { left, top }]}>
        ...
        <Pressable style={styles.row} onPress={onReorder}>
          <Text style={styles.rowText}>순서변경</Text>
        </Pressable>
        <View style={styles.divider} />
        <Pressable style={styles.row} onPress={onDelete} disabled={!canDelete}>
          <Text style={[styles.rowText, styles.dangerText, !canDelete && styles.disabledText]}>삭제</Text>
        </Pressable>
      </View>
    </>
  );
